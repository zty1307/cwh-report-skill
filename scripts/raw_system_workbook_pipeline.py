...
def build_overseas_review_packet(
    ...
    return {
        ...
        "instructions": [
            ...
            "主要报道对象包括会议本身，或本次会议具体决策及其政策影响；必须结合全文主线判断，不能只因出现国常会、政策名称或投资数字就include。",
            "综合行情、开收市盘点、多消息合集若主体是股价涨跌或多项无关消息，本次会议仅作其中一个催化因素、背景注脚或孤立要闻，则exclude；明确引用会议消息也不自动改变这一结论。",
            "不得按财经题材或合集标题一概排除：正文确实围绕本次决策展开政策内容、实施机制或直接影响分析时可include。review_reason须说明全文主线与本次决策的关系，排除时说明为何会议提及不足以构成主要报道。",
            "落实或执行报道须在正文中明确关联本次会议的具体决策，不能把同一政策领域、另一场会议或一般领导人活动自动算本次会议报道；只用一句上级部署作背景而主线另有所指时exclude。",
...
def filter_public_top(
    ...
    review_packet = {
        ...
        "instructions": [
            ...
            "会议在综合时事文章中占一个独立章节，也不自动成为整篇主体；其他无关事件并列构成全文主线时exclude。review_reason须回答整篇是否围绕本次会议决策，而非只说会议是重要内容之一。",
