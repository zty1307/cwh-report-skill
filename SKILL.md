...
Establish each raw child workbook's topic identity before semantic research. Use explicit export titles, a traceable monitoring-task/event-ID mapping, or an explicitly user-confirmed convention that numbered child files follow the current communique's agenda order. Agenda order alone, filename sorting and similar quantities are not identity evidence. Preserve the user's confirmed convention instead of repeatedly asking, but never make it a universal default. Keep the actual basis in period metadata, preserve combined statistical groups, and do not request another analysis file from the user. If identity is genuinely unavailable, ask only for the missing correspondence and retain the unresolved state; never guess factual labels or use another period's displayed dashboard. A benchmark-assisted correction is separately labelled, not a production fallback.

For raw inputs without explicit metadata, the workbook node first uses `scripts/cwh_meeting_intake.py`: scan all original public-article rows for the current government communique, preserving complete bodies outside the TOP ranking; if absent use the supplied `--communique-url`. The shared model reads the full text to group the complete agenda, not a regex split of deployment verbs. `--confirm-agenda-order` records an actual user confirmation, not an automatic default. Check date, source hash, literal topic evidence, distinct child indices and complete topic coverage before labelling tables. A fetched page is `text_acquired`, not AI-read. The workbench exposes the fallback URL and the two statuses separately. Read [references/meeting_intake.md](references/meeting_intake.md) when using this entry.
...
- Each eligible claim needs a verified in-window publication time, immutable full-text snapshot and hash, one speaking subject, a continuous excerpt with exact offsets, and a substantive `formal_claim` (normally 50-120 characters (punctuation included; attribution and whitespace excluded), not a truncation cap). Extract distinct judgments separately even from one speaker; one judgment may use several short sentences. Search snippets are not source snapshots.
...
- In bounded mode, normally select 4-6 attributed excerpts across 2-3 judgments per topic; excerpt count is not distinct-subject count. These are editorial references, not quotas or hard deletion limits. Add material only for indispensable information gain; keep other valid candidates as `formal_use=reserve`. The existing 12-subject safety cap and traceable thin-evidence exceptions remain, never pad to meet them. Do not shrink discovery or full-text reading. Exhaustive mode maps every eligible voice into prose.
...
- Preserve monitoring-system images as source artifacts. Report trend/topic charts must instead use the baseline-locked templates in `config/report_visual_style.v1.json` and `scripts/cwh_report_visuals.py`; only current date, full topic labels and counts vary. The nine-column/two-header-row topic table uses `templates/topic_table.xml`, including blank sentiment cells when no reviewed ratio is available. Word and workbench use identical generated chart bytes. Public evidence does not change workbook counts except through the audited in-window overseas-media reconciliation stage.
...
Use the shipped `templates/cwh_fill_in_template.docx` for Word output; read [references/word_fill_in_template.md](references/word_fill_in_template.md) when developing or repairing template filling. The renderer fills 11 named slots and retains fixed headings. Do not ask a model to recreate Word layout or invent a replacement template. Scripts own fonts, tables, charts, paragraph boundaries and placeholder checks; models own evidence selection and semantic grouping. Physical paragraph splitting never certifies source-order recovery as completed editorial grouping.

Read [references/formal_report_rules.md](references/formal_report_rules.md) before rendering. General evidence/writing rules are in `config/formal_writing_rules.v1.json`. The user-approved fixed wording and configurable chair default are in `config/editorial_template.v1.json`; `cwh_writing_rules.writing_rules()` merges them for every model and renderer. Do not ask models to rewrite fixed prose. Editorial defaults are not measurements or source certifications. Current sourced meeting metadata takes precedence; dates, agendas, topics, figures and quotations must come from the current run. The shipped Word is the reviewed fill-in skeleton, not a model-authored layout.
...
Use the shared claim-composition and cluster-structure rules in the current author task: concrete judgment plus its actual reasoning or conditions, not article introductions. Cluster count follows substantive evidence (usually 2-3, up to 5-6 distinct judgments when supported), never a mandatory topic checklist. Baseline style is useful; its dates, sentiment ratios, people, slogans and unsupported attention claims are not current facts.

Scripts share one rule configuration for Word and Markdown: opening, chapter names, propagation sentences, source ordering, attribution, numbering, comment lead, hotword frames and overseas transitions. Topic counts and numbering are dynamic; never discard the ninth group or shorten a qualification to fit a template. Source-backed `meeting.chair_name` and `meeting.chair_source` override the configurable approved chair default; use the neutral opening only when neither is available. Keep explicitly approved fixed editorial phrases in `config/editorial_template.v1.json`; do not infer additional names, publishers, peak dates, placement or factual-report majorities from historical examples. Editorial wording never certifies those claims as measured findings.
