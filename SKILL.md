...
Use the model-neutral `bounded_60m` profile in `config/execution_policy.v1.json` by default. It has a 3600-second wall-clock budget, per-stage timeouts, bounded query/page limits, and a delivery buffer. Time starts at the first `run` and includes worker waits and retries; `run` and `invalidate` do not reset it. An exhausted budget records `time_budget_exhausted`; use a new job directory for a new timed test. Use `exhaustive` only when the user explicitly prefers uncapped research over the one-hour target.
...
Each AI stage receives one `tasks/<stage>.json` generated by `scripts/cwh_model_contract.py`. The worker may write only paths in `declared_outputs`. It must not:
...
- Read `config/source_registry.v1.json` as a priority seed, not an allowlist. Follow the grouped lanes and caps emitted by `research_plan.json` in `bounded_60m`; only `exhaustive` expands every required source separately.
...
- `scripts/normalize_cwh_analysis.py` regenerates `cluster.details` from verified ordered atomic claims. Do not preserve or trust model-written paragraph wording.
