...
Record `topic_mapping_basis` for the identity of each original statistical group. A meeting agenda lists decisions, not monitoring-query identities: never infer labels solely from agenda order, filename sorting or similar totals. An explicit user-confirmed convention that numbered child queries follow the current agenda is valid identity evidence when paired with full communique understanding and complete count/index checks; preserve that confirmation without making it a global default. See `meeting_intake.md`. Preserve combined groups; do not omit an agenda because another group was split. If the basis says inferred/assumed/推断/猜测 or status is unconfirmed, obtain source identity or user confirmation before factual labels. The host rejects known-unconfirmed mappings (`topic_mapping_requires_confirmation`); after genuine confirmation record `topic_mapping_confirmed: true`, a confirmed status and actual basis. Explicit `child_file_indices` supports named heat workbooks only with confirmed mapping; source files and quantities are not renamed or rewritten. Legacy explicit titles remain compatible but are not independently certified. Historical baseline numbers never manufacture missing identities/counts.
