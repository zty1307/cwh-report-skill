...
8. Use the bundled cloud silhouette by default with transparent background, `#0D4E6D` text, `-15` degree rotation, PNG output and no white margin. Prefer `D:\第九周\腾讯字体.otf` when it exists; allow run metadata to override the font or mask path.
