...
8. Use the bundled cloud silhouette by default with transparent background, `#0D4E6D` text, `-15` degree rotation, PNG output and no white margin. Supply an organization-approved font through run metadata `wordcloud.font_path` or the process-local `CWH_CJK_FONT`; do not depend on a particular period's directory. The public package's Noto fallback is not the internal Tencent typeface. Preserve the actual resolved font in the render audit; do not claim a fallback matches the reference font. Font redistribution must respect the public repository's licensing boundary.
