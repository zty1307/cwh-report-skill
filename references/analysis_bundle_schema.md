...
- Ingest the raw stage's complete `public_article_evidence` corpus independently of `公众TOP`. The appendix TOP review batch and final TOP10 are never the domestic-viewpoint candidate pool. Exhaustive mode requires a decision for every row. Bounded mode may use the controller's hash-matched `public_corpus_index.json`: review at least `min(12, topic corpus size)` complete articles per topic, record their `reviewed_record_ids`, `retained_record_ids` and `excluded` reasons, and retain the rest as `deferred_record_ids` with `deferral_reason`. The controller computes missing deferral lists from the complete source index. Deferred rows are not reviewed, rejected, eligible or counted as evidence of saturation. This never waives independent voices, actual source-route checks, exact source excerpts or independent semantic review.
- Raw-monitoring candidates may supply `raw_evidence_record_id` without recopying the full article. Missing original URL/title/source/time and snapshot text are filled from the exact source row before structural completion; existing inconsistent values are not overwritten. A candidate's raw URL and snapshot text must match that record. Index shortlists are a deterministic reading order, not a semantic relevance judgment.
...
- Cluster the eligible pool first. In either bounded profile, mark representative voices `formal_use=selected` and additional valid candidates `formal_use=reserve` with a reason; normally select 4-6 attributed excerpts across 2-3 judgments, not a quota of people; additions need indispensable information gain. The existing 12-independent-subject safety cap remains, not a target. Selected candidates must map to a formal cluster and final outputs; reserve candidates remain in the research audit. In `exhaustive`, every eligible candidate maps to formal prose. Exact mirrors remain `duplicate`.
...
- Normally select 2-3 distinct judgment groups per topic, with additional groups only for indispensable information gain (within the existing six-group bounded safety ceiling). Do not treat this as a minimum quota. A one-cluster result retains the existing `single_cluster_exception` with actual search evidence, without inventing a second judgment.
- Each mature cluster normally cites two complementary independent voices, with an audited single-voice exception or a substantive third-source addition. Preserve one complete attributed claim per voice, normally 50-120 characters (punctuation included, attribution and whitespace excluded; a soft target). A thinner cluster must include `thin_cluster_exception` backed by completed search evidence.
...
- Each speaking subject is checked separately, including multiple people quoted in one article. Each attributed proposition should normally retain 50-120 characters (punctuation included, attribution and whitespace excluded; a soft target) of substantive source meaning. Claims below 30 Chinese characters fail the formal gate and must be expanded from the same source excerpt, combined with related wording from that source, or removed. Generic padding is not allowed. Bare noun phrases such as `宏观分析人士` and `政策解读文章` are not attribution verbs.
...
- Set `interpretive_verified=true` only when an overseas item contains a verifiable analytical passage, and supply its continuous original `interpretive_excerpt`. Raw overseas review rejects missing/title-only bodies and interpretive inclusion without this exact source span; an AI-written summary never certifies itself. Every overseas row eligible for formal output must retain the original and provide reviewed Simplified Chinese fields (`title_cn_simplified`, `summary_cn_simplified`, `source_cn_simplified`). Traditional Chinese is normalized with the complete OpenCC conversion layer; foreign-language translation and the resulting names, policy terms, numbers and meaning are reviewed by AI. Traditional Chinese and foreign-language text may remain in evidence and the dashboard but never enters formal prose or appendix cells directly.
