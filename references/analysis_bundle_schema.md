        ...
        "clusters": [
          {
            ...
            "details": "named expert/media/self-media interpretation in formal report style",
...
- Read `config/source_registry.v1.json`. Search every `must_check` domestic media source for every topic before open search, and record `hit`, `no_relevant_result`, `access_failed` or `not_applicable`. Missing rows fail the gate; do not silently treat a search backend omission as no result.
...
- Do not stop domestic research because a minimum evidence count has been met. Continue different open-search query/source rounds until two consecutive rounds add zero material high-relevance independent viewpoints. Every round must preserve concrete `executions` with query ID/text, backend, route, timestamp, result count, URL snapshot and retained candidate IDs. Labels such as `general_open_search` or a self-reported zero count are not evidence and fail the formal gate.
...
- Cluster the complete eligible pool first. Every eligible independent candidate must be mapped to a formal cluster, reference its `candidate_id`, and appear in both the dashboard and formal prose. No mature cluster has an upper item cap. Exact mirrors and reposts use `decision=duplicate` and remain in structured audit only; they do not produce formal sentences, dashboard cards or visible sample-count increments. An eligible candidate left outside all formal clusters is a blocking completeness error.
...
- Domestic comment collection covers every workbook topic and every currently permitted comment adapter with topic-specific query families. It has no target quote count and never stops because one topic already supplied enough quotations. Preserve per-topic/per-platform `hit`, `no_relevant_result` or `access_failed`, query evidence and saturation; a final report may contain fewer topic groups only when the audit genuinely found no traceable substantive comments for the omitted topics.
...
- Each mature cluster should cite every eligible independent named source mapped to it; two voices are the maturity floor and no upper cap applies. Preserve a complete attributed claim for every voice, normally 45-120 Chinese characters, and let `details` grow with the eligible set. A thinner cluster must include `thin_cluster_exception` backed by the completed search audit. Combine complementary voices in one paragraph; do not pad the paragraph or repeat the same source through syndicated copies.
- Viewpoint `details` must name every independent speaking subject and explain what it argued. Use full verified institution/title/name for a person, or the concrete media/platform/account name when no person is identified. Do not write generic policy summaries, `专家认为`、`媒体关注`、`报道汇集某某等对……的分析`, `样本来源：……` or `公开网络补证`.
