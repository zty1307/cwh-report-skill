...
In bounded mode, normally select 4-6 attributed excerpts across 2-3 distinct judgments per topic. These are soft editorial references, not independent-person quotas or hard deletion limits. Additional excerpts need an indispensable new judgment, condition, suggestion or dissent; thin topics retain less without padding. The existing 12-subject and six-group safety caps remain, not targets. Keep additional valid material as audited reserves. Do not shrink discovery or full-text reading. Whole-report excerpt and paragraph totals depend on current topics and evidence, never historical totals. Exhaustive mode maps every eligible independent voice into prose.
...
The common writing unit across baselines is a concrete judgment with its source-supported reason, mechanism, condition or suggestion, not a report introduction. Use `viewpoint.claim_composition_rule` and `viewpoint.cluster_structure_rule` in both author transports. Retain specific policy objects, implementation actors and effect boundaries; do not compress actual methods into abstract phrases such as “improve mechanisms”. Suggestions remain suggestions; pending drafts remain pending. Order clusters for understanding, not by outlet class. Do not inherit baseline dates, ratios, fixed expert names, measured positive sentiment or rhetorical padding. The only fixed editorial defaults are those explicitly approved in `config/editorial_template.v1.json`; they do not alter evidence or measurements. Different comment reactions may coexist within a topic; the current grouping contract is not inferred from one baseline's numbered count.
...
Do not deduplicate by speaker name alone. One speaker can provide materially different judgments for different clusters; preserve each supported judgment and count the person only once in topic-level independent-source totals. Each statement still requires its own source mapping and independent review; the host must not merge unreviewed meanings or keep only the first statement merely because the attribution repeats.

Authoring, synthesis, local repair and independent review share the configured editorial eligibility criteria. Textual support does not itself establish suitability for the report; assess actual policy analysis, not financial vocabulary or the source channel. Following reviewer narrowing or exclusion, recheck headings against the complete retained, independently reviewed claims only. Original excerpts remain in the evidence audit but must not reintroduce a removed assertion into a heading. Missing or invalid heading review uses an explicitly audited neutral display fallback while preserving the original draft; it is not a successful semantic heading review.
...
Source fidelity is not a certificate that the source's own reasoning is correct. Apply `source_reasoning_quality_rule` to visible arithmetic contradictions, mixed units, assumptions presented as facts and unsupported policy-stage shifts. Record the specific concern instead of replacing the author's figures or inventing a better argument. Preserve separately supported suggestions, including criticism. The reviewer must distinguish exact textual support from formal-use eligibility; adding "the author thinks" cannot cure an obviously faulty premise. This is a bounded check of the supplied evidence, not a claim to have independently fact-checked every background statement on the web.
...
合法JSON不代表复核可用。正文审核的rationale必须是非空字符串，空值、错误类型、缺字段和重复/漏掉证据ID不得成为支持认证。宿主在标题返修及原文认证前检查这些字段；字段无效时，只在原独立复核阶段剩余预算允许时，对受影响条目做一次最长45秒的真实补核，保留其原ID、完整冻结摘录及必要对象语境，不重新审核其他正文和标题、不增加全程预算。补核可改变这些条目的判断，但不能给其他旧结论补写理由或重盖运行ID。逐条保留实际审核运行，删改后的认证包也须继承该来源。旧原答、缓存、摘要哈希及真实运行保留；原先标题仍归原始实际标题审核运行。重试仍无效就明确失败，不重复三次读取同一坏缓存当作三次新复核。

超过12条正文观点时，独立原文复核默认按至多8条的小批顺序处理；每条原ID、完整连续摘录和必要来源语境保持不变。脚本发现的跨题重复/共享原文比较必须在同一批同时看到双方，不为凑批大小截断关联组。逐条保留真实审核运行，不将最后一批的认证盖到前面各批。正文完成取舍后，再仅用实际保留的完整判断审核标题。所有批次、局部修补及标题仍共用原阶段剩余预算，缺少任何一条审核不能伪称全文已核验。
...
# Cross-model scope and truthful empty sections

Every impact, comparison or forecast must preserve the original effect object, population, range and strength. A judgment about differences in property values is not a judgment about overall differences between regions; adding “may” does not repair an expanded object. Authors, selected-field repair and fresh independent review use the same maintained writing rule.

A comment's stated condition is not interchangeable with its parent post's policy action. The real parent may establish topic relevance, but cannot add an action→condition causal step or a wish that the commenter did not express. A heading must summarize the original comment's actual judgment and attitude. Frozen labels, quotations and sentiment denominators remain unchanged.

When no quote is ready, fixed prose distinguishes verified actual samples without suitable approved quotations, unverified candidate clues, and no captured samples. Do not write “no comments obtained” merely because formal approval is zero. This notice never creates a sentiment ratio or semantic approval. The final Word audit checks that eligible approved domestic quotations actually appear in the body section; appendix or JSON presence cannot mask a missing displayed quotation. Explicit user prose overrides are recorded as not applicable, not certified by this coverage check. Coverage is separate from semantic correctness and physical pagination.
